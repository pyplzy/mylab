package usi.dbdp.portal.help.dao.impl4oracle;
...
@OracleDb
@Repository
public class HelpDaoImpl extends JdbcDaoSupport4oracle implements HelpDao{
	...
	public Long saveFeedback(final FeedbackDto feedbackDto) {
		final String sql = "insert into ptl_feedback(feedback_id,feedback_title,feedback_info,user_id,user_name,org_id,org_name,contact_tel) values(ptl_feedback_seq.nextval,?,?,?,"+OraclePasswdUtil.getEncryptStr("?")+",?,"+OraclePasswdUtil.getEncryptStr("?")+","+OraclePasswdUtil.getEncryptStr("?")+")";
	...
	public List<FeedbackDto> getFeedbackList(FeedbackDto feedbackDto, PageObj pageObj) {
		String sql = "select feedback_id,feedback_title,feedback_info,feedback_time,state,user_id,"+OraclePasswdUtil.getDecryptStr("user_name")+" AS user_name,org_id,"+OraclePasswdUtil.getDecryptStr("org_name")+" AS org_name,"+OraclePasswdUtil.getDecryptStr("contact_tel")+" AS contact_tel from ptl_feedback t where 1=1 ";
		List<Object> param = new ArrayList<Object>();
		if(StringUtils.hasText(feedbackDto.getFeedbackTitle())){
			sql += "and t.feedback_title like CONCAT(CONCAT('%',?),'%')   ";
			param.add(feedbackDto.getFeedbackTitle());
		}
		if(StringUtils.hasText(feedbackDto.getUserName())){
			sql += "and "+OraclePasswdUtil.getDecryptStr("t.user_name ")+" like CONCAT(CONCAT('%',?),'%')   ";
		...
		return this.queryByPage(sql,param.toArray(),new RowMapper<FeedbackDto>() {
					...
					public FeedbackDto mapRow(ResultSet rs, int rowNum) throws SQLException {
						...
						item.setFeedbackDate(rs.getString("feedback_time"));
...
//						item.setReplyTime(rs.getTimestamp("reply_time"));
	...
	public FeedbackDto getFeedbackDetail(Long feedbackId) {
		String sql = "select feedback_id,feedback_title,feedback_info,question_type,reply_content,reply_time,state,user_id,"+OraclePasswdUtil.getDecryptStr("user_name")+" AS user_name,org_id,"+OraclePasswdUtil.getDecryptStr("org_name")+" AS org_name,"+OraclePasswdUtil.getDecryptStr("contact_tel")+" AS contact_tel from ptl_feedback t where feedback_id=? ";
