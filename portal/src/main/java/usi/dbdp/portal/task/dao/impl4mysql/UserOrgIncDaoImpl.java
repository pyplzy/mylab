...
@MysqlDb
@Repository
public class UserOrgIncDaoImpl extends JdbcDaoSupport4mysql implements UserOrgIncDao {

	@Override
	public User getUserByUserIdForInc(Long id) {
		String sql="select " +
				"  a.create_time,a.duration,a.gender,a.id,a.lst_err_pwd_time,a.mobile_no,a.password,a.pwd_err_cnt,a.pwd_last_mod_time,a.del_flag, " + 
				"  a.user_id,a.user_name,a.login_id,a.user_type " + 
				"  from uic_user a  " + 
				"  where a.id=? ";
	...
	public boolean insertIncUser(final User user, final String incType) {
		String sql="insert into uic_user_inc (id,user_id,user_name,login_id,gender,mobile_no,user_type,del_flag,password,duration,pwd_last_mod_time,lst_err_pwd_time,pwd_err_cnt,create_time,inc_type,occur_time) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,now())";
	...
	public boolean insertIncOrg(final Org org, final String incType) {
		String sql="insert into uic_org_inc (org_id,org_code,org_name,p_org_id,org_grade,administrative_grade,org_code_seq,org_id_seq,org_name_seq,display_order,is_leaf,del_flag,org_memo,inc_type,occur_time) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,now())";
	...
	public int deleteUicUserIncByDate(String date) {
		String sql = "delete from uic_user_inc WHERE OCCUR_TIME <=date_format(?,'%Y-%m-%d %H:%i:%s') ";
		return this.getJdbcTemplate().update(sql, new Object[]{date});
	}
