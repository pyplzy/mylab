...
@OracleDb
@Repository
public class NewsDaoImpl extends JdbcDaoSupport4oracle implements NewsDao{
	...
	public long insertNews(final NewsDto news){
		...
		this.getJdbcTemplate().update(new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(Connection con) throws SQLException {
				String sql = "insert into ptl_news (news_id,title, cover_summary,cover_pic_url,content, state, "
						+ "user_id, release_time, reply_flag,user_name) "
						+ "values (ptl_news_seq.nextval,?, ?, ?, ?, ?, ?, ?, ?,"+OraclePasswdUtil.getEncryptStr("?")+")";
	...
	public int updateNews(final NewsDto news){
		String sql =  "update ptl_news set title=?, content=?, state=?, create_time=sysdate, "
	...
	/*public List<NewsDto> queryPublishedByUserId(NewsDto newsDto, PageObj pageObj ) throws ParseException{
		String sql = "SELECT *"
				...
				+ "	 to_char(T.RELEASE_TIME, 'yyyy-mm-dd hh24:mi:ss') RELEASE_TIME,"
	...
	public List<NewsDto> queryPublishedByUserId(NewsDto newsDto, PageObj pageObj ) throws ParseException{
		...
		String sql = "SELECT *"
				...
				+ "	  RELEASE_TIME,"
				+ "  T.READ_CNT,"
				+ OraclePasswdUtil.getDecryptStr("T.USER_NAME")+" AS user_name,"
				+ "  (SELECT COUNT(*) FROM PTL_NEWS_COMMENT R WHERE R.NEWS_ID = T.NEWS_ID) REPLY_NUM,T.REPLY_FLAG "
				+ "   FROM ptl_news T WHERE 1=1 ";
		sql += "     AND T.STATE = 1) T1 WHERE 1 = 1 ";
		if (!"-1".equals(newsDto.getUserId()) || !CommonUtil.hasValue(newsDto.getUserId())) {
	...
	public void batchUpdateToPublishedStatus(final long[] bulletinIds){
		String sql = "UPDATE ptl_news SET STATE = 1, RELEASE_TIME = sysdate WHERE NEWS_ID = ?";
	...
	public void batchUpdateToDraftsStatus(final long[] bulletinIds){
		String sql = "UPDATE PTL_BULLETIN SET STATE = 0, RELEASE_TIME = NULL, CREATE_TIME = sysdate, "
	...
	public NewsDto queryNewsById(Long newsId){
		String sql = "select t1.news_id,t1.user_id,t1.title,t1.content,t1.reply_flag,t1.release_time,t1.cover_pic_url,"
				+ OraclePasswdUtil.getDecryptStr("t1.user_name")
				+" AS user_name,t1.read_cnt,t1.cover_summary,t1.org_id,"
				+OraclePasswdUtil.getDecryptStr("t1.org_name")+" AS org_name,t1.create_time "
	...
	public List<BulletinDto> queryNoticeByPage(Date date , PageObj pageObj, Long province){
		...
		if(0==province){   //查询系统所以公告
			sql = "SELECT T.BULLETIN_ID, "+OraclePasswdUtil.getDecryptStr("u.user_name")+" as user_name, "
					+ "       T.TITLE,"
					+ "       T.STICK_UP,"
					+ "       (SELECT COUNT(*)"
					+ "          FROM PTL_FILE T1"
			...
			sql = 
					"SELECT  T.BULLETIN_ID, "+OraclePasswdUtil.getDecryptStr("u.user_name")+" as user_name, " +
							"       T.TITLE, " + 
							"       T.STICK_UP, " + 
							"       (SELECT COUNT(*) " + 
							"          FROM PTL_FILE T1 " + 
	...
	public List<BulletinDto> queryNoticePublishedByPage(PageObj pageObj){
		String sql = "SELECT *"
				+ "  FROM (SELECT  "+OraclePasswdUtil.getDecryptStr("u.user_name")+" as user_name,  "
				+ "	T.BULLETIN_ID,"
				+ "               T.TITLE,"
				+ "				T.STICK_UP,"
				+ "               to_char(T.CREATE_TIME, 'yyyy-mm-dd hh24:mi:ss') CREATE_TIME,"
				+ "               to_char(T.RELEASE_TIME, 'yyyy-mm-dd hh24:mi:ss') RELEASE_TIME,"
				+ "               to_char(T.LOSE_TIME, 'yyyy-mm-dd hh24:mi:ss') LOSE_TIME,"
				+ "               T.READ_CNT,"
				+ "               (SELECT COUNT(*) "
				+ "                  FROM ptl_file T1"
				+ "                 WHERE T1.GROUP_CODE = 'NOTICE' "
				...
				+ " T.STATE=1 AND (T.LOSE_TIME > sysdate OR T.LOSE_TIME IS NULL)  ) T1 ";
...
//			
//			"select t0.* from ptl_news t0 "+
//			"where t0.state=1 order by t0.release_time desc limit 5";
//			sql = "SELECT * FROM " +
//				"   (select b.*,ROWNUM rn from (select t0.* from ptl_news t0 " + 
//				"where t0.state=1 order by t0.release_time desc) b) " + 
//				"WHERE rn <= 5 ";
//			params = new Object[]{};
//		} else {
...
//			sql = "SELECT * FROM " +
//							"   (select b.*,ROWNUM rn from (select t0.* from ptl_news t0,uic_org t1 " + 
//							"where t0.state=1 and t0.org_id=t1.org_id and t1.org_id_seq like concat((select o.org_id_seq from uic_org o where o.org_id = ?),'%') " + 
//							"order by t0.release_time desc) b) " + 
//							"WHERE rn <= 5 ";
//			params = new Object[]{province};
//		}
		sql = "select t0.* from ptl_news t0 where t0.state=1 and ROWNUM<=5 order by t0.release_time desc ";
