...
@OracleDb
@Repository
public class LogDaoImpl extends JdbcDaoSupport4oracle implements LogDao{
	...
	public List<LoginLogInfo> getUicLoginLogInfo(LoginLog uicLoginLog ,PageObj pageObj ){
		String sql="select  ll.login_id , ll.user_id , ll.login_ip , ll.login_time , ll.is_success , "+OraclePasswdUtil.getDecryptStr("u.user_name")+" as user_name , t.org_name   "
				+ " from uic_login_log ll left join  uic_user u on ll.user_id =u.user_id left join (select "+OraclePasswdUtil.getDecryptStr("o.org_name")+" AS org_name,uo.id from uic_org o ,uic_user_has_org uo where uo.org_id=o.org_id and uo.type=1 ) t   on u.id = t.id "
				+ " where  login_time>=to_date(concat(?,' 00:00:00'),'yyyy-mm-dd hh24:mi:ss') and login_time<=to_date(concat(?,' 23:59:59'),'yyyy-mm-dd hh24:mi:ss')  ";
	...
	public List<ChangeLogInfo> getUicChangeLogInfo(ChangeLogInfo uicChangeLogInfo ,PageObj pageObj ){
		String sql="select  cl.change_id , cl.user_id ,cl.opt_obj, cl.opt_type , cl.opt_ip , cl.opt_content , cl.opt_time "
				...
				+ " where  cl.opt_time>=to_date(?,'yyyy-mm-dd hh24:mi:ss') and cl.opt_time<=to_date(?,'yyyy-mm-dd hh24:mi:ss') ";
	...
	public boolean saveLoginLogInfo(final LoginLog loginLog){
		String sql="insert into uic_login_log(login_id,user_id,login_ip,session_id,is_success)"
				+ " values (uic_login_log_seq.nextval,?,?,?,?) ";
	...
	public boolean saveChangeLogInfo(final ChangeLog changeLog){
		String sql="insert into uic_change_log (change_id,user_id,opt_obj,opt_type,opt_ip,opt_content,opt_time) "
				+ " VALUES(uic_change_log_seq.nextval,?,?,?,?,?,sysdate) ";
		return  this.getJdbcTemplate().update(sql,new PreparedStatementSetter() {
			@Override
			public void setValues(PreparedStatement ps) throws SQLException {
	...
	public boolean saveLogoutInfo(String userId) {
		String sql="SELECT LOGIN_ID FROM( SELECT * FROM UIC_LOGIN_LOG WHERE USER_ID=? AND IS_SUCCESS=1 ORDER BY LOGIN_TIME DESC) WHERE ROWNUM=1";
		Long loginId=this.getJdbcTemplate().queryForObject(sql, Long.class, userId);
		String updateSql="UPDATE UIC_LOGIN_LOG SET LOGOUT_TIME=sysdate WHERE LOGIN_ID=?";
