...
@MysqlDb
@Repository
public class PeopleOrgDataImpl  extends JdbcDaoSupport4mysql  implements PeopleOrgDataDao{

	@Override
	public List<PeopleOrgDataDto> getUsersForNowOrg(Long orgId, String userName,PageObj pageObj) {
		String sql="select u.id,u.user_id,u.user_name,o.org_name,pri.org_id as isadd,o.org_id,pri.org_id as manageOrgId,uor.org_name as manageOrgName  from uic_org o, uic_user_has_org uo,uic_user u left join  uic_org_data_pri pri  on u.user_id=pri.user_id  left join uic_org uor  on (uor.org_id=pri.org_id    and uor.del_flag=0) where   uo.id=u.id and uo.org_id=o.org_id and uo.type=1 and u.del_flag=0 and o.del_flag=0 ";
	...
	public List<UserInfo> getUsersByOrgId(Long adminOrgId, Long orgId,String adminId, String userId, PageObj pageObj) {
		String sql="select u.id ,u.user_id ,u.login_id ,u.user_name ,o.org_name_seq ,o.org_id   "
