...
/**
 * @author zhqnie
 * @version 2016年10月18日 下午5:30:33
 * 说明
 */
@MysqlDb
@Repository
public class RoleManageDaoImpl extends JdbcDaoSupport4mysql implements RoleManageDao{

	@Override
	public List<OrgDto> getProvinces(Long province) {
		String sql = "";
		if(province==0){
			sql = " SELECT org_id,org_name FROM uic_org  WHERE org_grade = 2 AND DEL_FLAG = 0 ";
			...
			sql = " SELECT org_id,org_name FROM uic_org  WHERE org_grade = 2 AND DEL_FLAG = 0 and org_id = ?";
	...
	public List<OptDto> qryOptsByRoleId(long roleId, long resourceId,
		...
		if(CommonUtil.hasValue(optName)){
			sql += " and t.PRIVILEGE_NAME  like  concat('%',?,'%') ";
			list.add(optName);
		}
