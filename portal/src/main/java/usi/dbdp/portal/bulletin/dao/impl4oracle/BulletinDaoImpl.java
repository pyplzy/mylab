...
@OracleDb
@Repository
public class BulletinDaoImpl extends JdbcDaoSupport4oracle implements BulletinDao{
	...
	public long insertNotice(final PtlBulletin ptlBulletin){
		...
		this.getJdbcTemplate().update(new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(Connection con)
					throws SQLException {
				String sql = " INSERT INTO ptl_bulletin (BULLETIN_ID,TITLE, CONTENT, STATE, READ_CNT, "
						+ "CREATE_STAFF, CREATE_TIME, LOSE_TIME, RELEASE_TIME, REPLY_FLAG) "
						+ " VALUES (ptl_bulletin_seq.nextval,?, ?, ?, 0, ?, sysdate, ?, ?, ?)";
	...
	public int updateNotice(final PtlBulletin ptlBulletin){
		String sql =  "UPDATE ptl_bulletin SET TITLE = ?, CONTENT = ?, STATE = ?, CREATE_TIME = sysdate, "
	...
	public List<BulletinDto> queryNoticePublishedByStaffId(long createStaff, String offline, PageObj pageObj ){
		String sql = "SELECT *"
				...
				+ "               T.READ_CNT,"
				+ "               T.STATE,"
				+OraclePasswdUtil.getDecryptStr("T2.USER_NAME")+" as USER_NAME,"
				+ "               CASE T.STATE"
				+ "                 WHEN 2 THEN 'y' "
				+ "                 ELSE "
				+ "                  (CASE WHEN T.LOSE_TIME < sysdate THEN 'y' "
				+ "                    ELSE 'n' "
				+ "                  END) "
				+ "               END OFF_LINE, "
				+ "               (SELECT COUNT(*) "
	...
	public void batchUpdateToPublishedStatus(final long[] bulletinIds){
		String sql = "UPDATE PTL_BULLETIN SET STATE = 1, RELEASE_TIME = sysdate WHERE BULLETIN_ID = ?";
	...
	public BulletinDto queryNoticeById(Long bulletinId){
		String sql = "SELECT 	t1.read_cnt,	"
				...
				+ "       T1.CONTENT,"
				+ "       T1.REPLY_FLAG,"
				+ "       to_char(T1.LOSE_TIME, 'yyyy-mm-dd hh24:mi:ss') LOSE_TIME,"
				+ "       to_char(T1.RELEASE_TIME, 'yyyy-mm-dd hh24:mi:ss') RELEASE_TIME,"
				+ "       T1.REPLY_FLAG,"
				+OraclePasswdUtil.getDecryptStr("T2.USER_NAME")+" as USER_NAME"
//				+ "       CREATE_NAME "
...
//					+ "           AND T1.RELATION_ID = T.BULLETIN_ID"
//					+ "           AND T1.IS_DEL = 0) FILE_NUM,"
//					+ "       to_char(T.RELEASE_TIME, 'mm-dd') RELEASE_TIME "
	...
	public List<BulletinDto> queryNoticeByPage(Date date , PageObj pageObj, Long province){
		...
		if(0==province){   //查询系统所以公告
			sql = "SELECT T.BULLETIN_ID, "+OraclePasswdUtil.getDecryptStr("u.user_name")+" as user_name, " 
					...
					+ "       to_char(T.RELEASE_TIME, 'mm-dd') RELEASE_TIME "
					+ "  FROM PTL_BULLETIN T , uic_user u  " + " WHERE T.STATE = 1 and t.create_staff=u.id "
					+ "   AND (T.LOSE_TIME > ? OR T.LOSE_TIME IS NULL) and rownum<=5  "
					+ " ORDER BY T.STICK_UP DESC, T.RELEASE_TIME DESC";
		}else{  //查对应省份公告
			sql = 
		...
		return this.getJdbcTemplate().query(sql, new Object[]{date},new RowMapper<BulletinDto>() {
