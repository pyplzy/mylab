...
public class AttachmentDaoImpl extends JdbcDaoSupport4oracle implements AttachmentDao{
	...
	public long saveAttachment(final PtlFile ptlFile){
		...
		this.getJdbcTemplate().update(new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(Connection con)
				...
				String sql = "INSERT INTO ptl_file(FILE_ID,FILE_NAME,ABSOLUTEPATH,FILE_SIZE,FILE_TYPE,"
						+ "STAFF_ID,FILE_TIME,GROUP_CODE,RELATION_ID,DOWNLOAD_TIMES,IS_DEL)"
						+ " VALUES(ptl_file_seq.nextval,?,?,?,?,?,?,?,?,?,?)";
	...
	public int savePtlFileOpLog(PtlFileOpLog ptlFileOpLog){
		String sql = "INSERT INTO PTL_FILE_OP_LOG(OP_LOG_ID,FILE_ID,"
				+ "STAFF_ID,OPERATOR_NAME,ORG_ID,ORG_NAME,"
				+ "OP_TIME,OP_TYPE) "
				+ "VALUES(ptl_file_op_log_seq.nextval,?,?,"+OraclePasswdUtil.getEncryptStr("?")+",?,"+OraclePasswdUtil.getEncryptStr("?")+",?,?)";
	...
	public void batchSavePtlFileOpLog(final PtlFileOpLog[] ptlFileOpLog){
		String sql = "INSERT INTO PTL_FILE_OP_LOG(OP_LOG_ID,FILE_ID,STAFF_ID,OPERATOR_NAME,ORG_ID,ORG_NAME,OP_TIME,OP_TYPE) "
				+ "VALUES(ptl_file_op_log_seq.nextval,?,?,"+OraclePasswdUtil.getEncryptStr("?")+",?,"+OraclePasswdUtil.getEncryptStr("?")+",?,?)";
