...
@OracleDb
@Repository
public class AddressBookManagerDaoImpl extends JdbcDaoSupport4oracle implements AddressBookManagerDao{
	...
	public int updatePersonInAddressBook(PtlAddressBook ptlAddressBook) {
		String sql = "update ptl_address_book set name="+OraclePasswdUtil.getEncryptStr("?")+",title=?,administrative_level=?,deparment=?,email=?,gender=?,\n" +
						"company=?,mobile_no="+OraclePasswdUtil.getEncryptStr("?")+",fixed_line_tel="+OraclePasswdUtil.getEncryptStr("?")+",user_cata=?,head_img=?,user_id=?\n" + 
	...
	public int addAddressBookRecord(PtlAddressBook ptlAddressBook) {
		String sql = "insert into ptl_address_book(address_book_id,name,title,administrative_level,deparment,email,gender,company,mobile_no,fixed_line_tel,user_cata,head_img) "+
				"values(ptl_address_book_seq.nextval,"+OraclePasswdUtil.getEncryptStr("?")+",?,?,?,?,?,?,"+OraclePasswdUtil.getEncryptStr("?")+","+OraclePasswdUtil.getEncryptStr("?")+",?,?)";
		return this.getJdbcTemplate().update(sql, new Object[] {ptlAddressBook.getName(),ptlAddressBook.getTitle(),ptlAddressBook.getAdministrativeLevel(),
				ptlAddressBook.getDeparment(),ptlAddressBook.getEmail(),ptlAddressBook.getGender(),ptlAddressBook.getCompany(),
				ptlAddressBook.getMobileNo(),ptlAddressBook.getFixedLineTel(),ptlAddressBook.getUserCata(),ptlAddressBook.getHeadImg()});
	...
	public int[] batchAddPersonInfos(final List<PtlAddressBook> ptladdressbooks){
		...
		String addSql="insert into ptl_address_book(address_book_id,name,title,administrative_level,deparment,email,gender,company,mobile_no,fixed_line_tel,user_cata,head_img,user_id) values(ptl_address_book_seq.nextval,"+OraclePasswdUtil.getEncryptStr("?")+",?,?,?,?,?,?,"+OraclePasswdUtil.getEncryptStr("?")+","+OraclePasswdUtil.getEncryptStr("?")+",?,?,?)";
