...
public class OrgDaoImpl extends JdbcDaoSupport4oracle implements OrgDao{
		...
		this.getJdbcTemplate().update(new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(Connection con)
				...
				String hsql = " INSERT INTO uic_org ("
						  ...
						  +" ) "
						  +" VALUES"
						  +" (uic_org_seq.nextval, ?, "+OraclePasswdUtil.getEncryptStr("?")+", ?, ?, ?, ?, ?, ?, "+OraclePasswdUtil.getEncryptStr("?")+", ?,?,?) ";
	...
	public int saveOrgSeq(final String orgIdSeq, final String orgNameSeq, final long orgId) {
		String sql = "update uic_org  set ORG_ID_SEQ = ? , ORG_NAME_SEQ = "+OraclePasswdUtil.getEncryptStr("?")+" where ORG_ID = ? ";
	...
	public int updateOrg(final Org org) {
		String sql =" update uic_org set"
				  +" ORG_NAME = "+OraclePasswdUtil.getEncryptStr("?")+","
				  +" P_ORG_ID = ?,"
				  +" ORG_GRADE = ?,"
				  +" DISPLAY_ORDER = ?,"
				  +" del_flag = ?,"
				  +" ORG_MEMO = ?,"
				  +" ORG_ID_SEQ = ?,"
				  +" ORG_NAME_SEQ = "+OraclePasswdUtil.getEncryptStr("?")+"," 
				  +" IS_LEAF = ?,"  
				  +" ADMINISTRATIVE_GRADE = ?,"  
				  +" ORG_CODE = ? ,"  
				  +" ORG_CODE_SEQ = ? "  
	...
	public List<Org> queryOrgById(long orgId){
		String sql = "SELECT "
				  ...
				  +" ORG_CODE,"
				  +" ORG_CODE_SEQ,"
				  +OraclePasswdUtil.getDecryptStr("ORG_NAME")+" as ORG_NAME,"
				  +" P_ORG_ID,"
				  +" ORG_GRADE,"
				  +" DISPLAY_ORDER,"
				  +" DEL_FLAG,"
				  +" ORG_MEMO,"
				  +" ORG_ID_SEQ,"
				 + OraclePasswdUtil.getDecryptStr("ORG_NAME_SEQ")+" as ORG_NAME_SEQ," 
				  +" ADMINISTRATIVE_GRADE," 
				  +" (case IS_LEAF when 0 then 'true' else 'false' end ) is_parent ,"  
				  +" IS_LEAF"  
				  +" FROM UIC_ORG"
				  +" WHERE ORG_ID ="+orgId;
		List<Org> org=getJdbcTemplate().query(sql, new RowMapper<Org>(){
			@Override
	...
	public List<UserInfo> getAllUsersByOrgIdWithPage(String orgIdSeq, String userId, String userName,PageObj pageObj){
		String sql =    " SELECT a.ID," +
				"         a.USER_ID," + 
				OraclePasswdUtil.getDecryptStr("a.USER_NAME")+
				"         as USER_NAME," + 
				"         a.LOGIN_ID," + 
				"         a.GENDER," + 
				OraclePasswdUtil.getDecryptStr("a.MOBILE_NO")+
				"         as MOBILE_NO," + 
				"         a.USER_TYPE," + 
				"         b.ORG_ID," + 
				"         a.del_flag ," +
				...
				"         to_char(a.CREATE_TIME, 'yyyy-mm-dd hh24:mi:ss') CREATE_TIME," + 
				 OraclePasswdUtil.getDecryptStr(" b.ORG_NAME")+" as ORG_NAME," +
				"         uo.type ," +
				 OraclePasswdUtil.getDecryptStr("  b.ORG_NAME_SEQ")+" as ORG_NAME_SEQ " +
			
				"    FROM UIC_USER a , uic_org b, uic_user_has_org  uo  WHERE a.id = uo.id and uo.org_id=b.org_id  " + 
				"   and  a.del_flag=0  AND b.del_flag = 0 " + 
				"   AND  b.org_id_seq LIKE ? ";
		if(CommonUtil.hasValue(userId)){
			sql +=" and a.USER_ID like '%" + userId + "%' ";
		}
		if(CommonUtil.hasValue(userName)){
			sql +=" and "+OraclePasswdUtil.getDecryptStr("a.USER_NAME")+" like '%" + userName + "%' ";
		}
		//mysql汉字排序（按照汉字首字母拼音排序）
		sql += " ORDER BY a.USER_NAME";
		return this.queryByPage(sql, new Object[]{orgIdSeq}, new RowMapper<UserInfo>(){
			@Override
	...
	public List<UserInfo> getUsersByOrgIdWithPage(long orgId, String userId, String userName,PageObj pageObj){
		String sql = "     SELECT a.ID, " +
				"      a.USER_ID, " + 
				"      a.LOGIN_ID, " + 
				 OraclePasswdUtil.getDecryptStr(" a.USER_NAME")+" as USER_NAME,"+
				"      a.GENDER, " + 
				 OraclePasswdUtil.getDecryptStr(" a.MOBILE_NO")+" as MOBILE_NO,"+
				"      a.USER_TYPE, " + 
				"      b.ORG_ID, " +
				 OraclePasswdUtil.getDecryptStr(" b.ORG_NAME")+" as ORG_NAME," +
				"      a.STATUS,    "+
		...
		if(CommonUtil.hasValue(userName)){
			sql +=" and "+ OraclePasswdUtil.getDecryptStr("USER_NAME")+" like '%" + userName + "%' ";
		}
		sql += " ORDER BY "+OraclePasswdUtil.getDecryptStr(" a.USER_NAME");
	...
	public List<Org> getParentOrgsByOrgIdSeq(String orgIdSeq){
		String sql = "select o.org_id,"+OraclePasswdUtil.getDecryptStr(" o.ORG_NAME")+" as ORG_NAME,o.org_grade,o.administrative_grade from uic_org o where ? like concat(org_id_seq,'%') AND o.del_flag=0 order by o.org_grade";
	...
	public List<Org> getAllOrgsByUserId(String userId){
		String sql="select o.org_id,"+OraclePasswdUtil.getDecryptStr("o.ORG_NAME")+" as ORG_NAME ,o.p_org_id,o.org_grade,o.display_order,o.org_memo,o.org_id_seq,"+OraclePasswdUtil.getDecryptStr("o.org_name_seq")+" AS org_name_seq,o.is_leaf"+
						" from uic_org o"+
	...
	public void updateChildOrgSeq(String orgIdSeq, String newOrgSeqName, String oldOrgSeqname) {
		...
		String sql="SELECT UO.ORG_ID,"
				+ "('"+newOrgSeqName+"'|| SUBSTR("+OraclePasswdUtil.getDecryptStr("UO.ORG_NAME_SEQ")+","+oldSeqLenth+",LENGTH(UO.ORG_NAME_SEQ))) ORGNAMESEQ"
		...
		String updateSql="update UIC_ORG set ORG_NAME_SEQ="+OraclePasswdUtil.getEncryptStr("?")+" where ORG_ID=?";
