package usi.dbdp.uic.a2a.dao.impl4oracle;
...
@OracleDb
@Repository
public class AccountingDaoImpl  extends JdbcDaoSupport4oracle implements AccountingDao{
	/**
	 * 设置用户状态为停用状态
	 * @param userId 工号
		...
		this.getJdbcTemplate().update(new PreparedStatementCreator() {
			public PreparedStatement createPreparedStatement(Connection con)
				...
				String sql="insert into "
						+ "uic_user(id,user_id,user_name,gender,mobile_no,user_type,del_flag,password,duration,pwd_last_mod_time,pwd_err_cnt,create_time,login_id) "
						+ "values(uic_user_seq.nextval,?,"+OraclePasswdUtil.getEncryptStr("?")+",?,"+OraclePasswdUtil.getEncryptStr("?")+",?,?,?,90,sysdate,0,sysdate,?) ";
	...
	public boolean updateUser(final User user){
		String sql="update uic_user "
				+ "set user_name="+OraclePasswdUtil.getEncryptStr("?")+",gender=?,del_flag=?,password=?,pwd_last_mod_time=sysdate,lst_err_pwd_time=?,pwd_err_cnt=?,mobile_no = "+OraclePasswdUtil.getEncryptStr("?")+" ,user_type= ? ,create_time = ?,login_id=?   "
				+ "WHERE  user_id =? ";
		return this.getJdbcTemplate().update(sql,new PreparedStatementSetter() {
			@Override
			public void setValues(PreparedStatement ps) throws SQLException {
	...
	public boolean updateUserPasswordByLoginId(final String loginId,final String oldPassword, final String newPassword) {
		String sql="update uic_user set password=? , pwd_err_cnt = 0 ,  pwd_last_mod_time =sysdate  WHERE login_id=? and password=?";
		return this.getJdbcTemplate().update(sql, new PreparedStatementSetter(){
			@Override
			public void setValues(PreparedStatement ps) throws SQLException {
	...
	public User getUserByUserId(String userId){
		String sql="select " +
				"  a.create_time,a.duration,a.gender,a.id,a.lst_err_pwd_time,"+OraclePasswdUtil.getDecryptStr("a.mobile_no")+" as mobile_no,b.org_id,a.password,a.pwd_err_cnt,a.pwd_last_mod_time,a.del_flag, " + 
				"  a.user_id,"+OraclePasswdUtil.getDecryptStr("a.user_name")+" as user_name,a.login_id,a.user_type   ,"+OraclePasswdUtil.getDecryptStr("b.org_name")+" as org_name " + 
...
//		String sql = "select u.id,u.user_id,"+OraclePasswdUtil.getDecryptStr("u.user_name")+" as user_name,u.gender,"+OraclePasswdUtil.getDecryptStr("u.mobile_no")+" as mobile_no,u.user_type,o.org_id,"+OraclePasswdUtil.getDecryptStr("o.org_name")+" as org_name,o.org_id_seq,"+OraclePasswdUtil.getDecryptStr("o.org_name_seq")+" as org_name_seq,u.login_id,u.del_flag,u.status"
		String sql = "select u.id,u.user_id,"+OraclePasswdUtil.getDecryptStr("u.user_name")+" as user_name,u.gender,"+OraclePasswdUtil.getDecryptStr("u.mobile_no")+" as mobile_no,u.user_type,o.org_id,"+OraclePasswdUtil.getDecryptStr("o.org_name")+" as org_name,o.org_id_seq,"+OraclePasswdUtil.getDecryptStr("o.org_name_seq")+" as org_name_seq,u.login_id,u.del_flag"
